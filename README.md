...
URL du site : https://trouvezsarutobi.netlify.com/
...
| Données     | Le fichier `data.json` décrit 30 phases de jeu ou plus. Indiquez _En partie_ s'il en décrit plus de 15.                                                                                                                                   | fait      |
...
| Transitions | Les transitions internes au jeu (d'une étape à une autre) et externes (intro, fin...) sont différentes                                                                                                                                    | fait      |
| État        | Le choix du personnage et/ou de ses caractéristiques impacte l'aventure au moins une fois. Le personnage choisis se voit .                                                                                                                | fait      |
| État        | Une décision ou un événement aléatoire survenu pendant l'aventure a un impact sur la suite, au moins une fois. Pour se battre contre les boss, il faut tirer un nombre aléatoire                                                          | fait      |
| État        | Le choix du personnage et/ou ce qu'il s'est passé pendant l'aventure impactent l'écran de fin. Si le joueur gagne, l'écran est différent de quand il perd                                                                                 | fait      |
| État        | Au moins un service (classe de type `GameService`) est utilisé                                                                                                                                                                            | fait      |
| Sauvegarde  | Lorsqu'on recharge la page (`Ctrl + R`), le personnage, ses caractéristiques, les choix du joueur et tout le reste sont rétablis                                                                                                          | fait      |
| Sauvegarde  | Lorsqu'on quitte le jeu et qu'on revient plus tard à la page d'accueil (sur le même navigateur), il est possible de reprendre l'aventure où on l'avait laissée                                                                            | fait      |
